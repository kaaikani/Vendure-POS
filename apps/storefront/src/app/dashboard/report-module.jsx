...
import { FileText, Download, Trash2, Search, Wifi, WifiOff, CreditCard } from 'lucide-react';
export default function ReportModule() {
    ...
    const exportCSV = () => {
        ...
        const rows = filteredReports.map(r => `${r.invoiceId},${r.date},${new Date(r.timestamp).toLocaleTimeString('en-IN')},${r.saleType},${r.customer.name},${r.customer.phone},${r.items.length},${r.subtotal},${r.discount},${r.taxAmount},${r.grandTotal},${r.receivedAmount},${r.balance}`).join("\n");
    ...
    const modeBadge = (type) => {
        ...
        return (<span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider border ${cls}`}>
        {modeIcon(type)} {type}
      </span>);
    };
          ...
          <table className="w-full text-left whitespace-nowrap text-sm border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 font-bold">
              {filteredReports.map((r) => (<React.Fragment key={r.invoiceId + r.timestamp}>
                  ...
                  {/* Expanded Detail */}
                  {expandedId === r.invoiceId && (<tr className="bg-slate-50">
                        ...
                        <div className="p-6 border-l-4 border-teal-400">
                          ...
                          <table className="w-full text-xs">
                            ...
                            <tbody className="divide-y divide-slate-100">
                              {r.items.map((item, i) => (<tr key={i} className="bg-white">
