...
import { 
  ScanLine, Printer, Plus, Search, Tag, RefreshCw, CheckCircle, 
  Settings, Copy, FileText, AlertTriangle 
} from 'lucide-react';
...
export default function BarcodeModule() {
  ...
  const addToPrintQueue = (p: any) => {
    if (!p.barcode) return alert('Cannot print label without a barcode!');
    if (!printQueue.find(q => q.id === p.id)) {
       setPrintQueue([...printQueue, { ...p, copies: 1 }]);
    }
  };

  const updatePrintCopies = (id: string, delta: number) => {
          ...
          <div className="h-full flex flex-col bg-slate-50">
             ...
             <div className="flex-1 overflow-auto p-4 md:p-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {filtered.map(p => (
                     <div key={p.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex items-center justify-between group hover:border-emerald-300 transition-colors">
                        <div>
                           ...
                           <div className="mt-3 flex items-center gap-2">
                             {p.barcode ? (
                               <span className="flex items-center gap-2 bg-emerald-50 text-emerald-700 px-3 py-1 rounded-md text-xs font-black border border-emerald-200 font-mono tracking-widest"><CheckCircle size={14}/> {p.barcode}</span>
                             ) : (
                               <span className="flex items-center gap-2 bg-amber-50 text-amber-700 px-3 py-1 rounded-md text-xs font-bold border border-amber-200"><AlertTriangle size={14}/> No Barcode Assigned</span>
                             )}
                           </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-2 shrink-0 ml-4">
                           {!p.barcode && (
                              <button onClick={() => generateBarcode(p.id)} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-white rounded text-xs font-bold flex items-center justify-center gap-1 shadow-sm"><RefreshCw size={12}/> Generate Formatted</button>
                           )}
                           {p.barcode && (
                              <button onClick={() => addToPrintQueue(p)} className="px-3 py-1.5 bg-white border border-slate-300 hover:bg-emerald-50 hover:border-emerald-300 hover:text-emerald-700 text-slate-600 rounded text-xs font-bold flex items-center justify-center gap-1 transition-all"><Printer size={12}/> Add to Printer</button>
                           )}
                        </div>
                     </div>
                  ))}
          ...
          <div className="h-full flex overflow-hidden">
             ...
             <div className="w-[350px] bg-white border-r border-slate-200 flex flex-col shrink-0">
                ...
                <div className="flex-1 overflow-auto p-4 space-y-3 bg-slate-50/50">
                  {printQueue.length === 0 ? (
                     <div className="text-center py-10 text-slate-400 font-medium text-sm border-2 border-dashed border-slate-200 rounded-xl bg-white p-4">Select items from the "Assign" tab to print barcode labels.</div>
                  ) : printQueue.map(q => (
                     <div key={q.id} className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm text-sm">
             ...
             <div className="flex-1 bg-slate-200/50 flex flex-col p-6 overflow-hidden">
                ...
                <div className="flex-1 bg-white border border-slate-300 shadow-lg rounded-xl overflow-auto p-8 flex justify-center items-start">
                  ...
                  {printQueue.length === 0 ? (
                    <div className="m-auto text-slate-300 flex flex-col items-center">
                      <Printer size={64} className="mb-4 opacity-50"/>
                      <p className="font-black text-2xl">Preview Area Blank</p>
                    </div>
                  ) : (
                    <div className="grid gap-4 w-full max-w-2xl" style={{ gridTemplateColumns: printFormat === 'a4_sheet' ? 'repeat(3, 1fr)' : 'repeat(2, 1fr)' }}>
                       {printQueue.flatMap(q => Array(q.copies).fill(q)).map((q, idx) => (
                           <div key={idx} className="border-2 border-slate-900 border-dashed p-3 flex flex-col items-center text-center bg-white">
                              ...
                              <div className="flex h-10 w-full justify-center items-end overflow-hidden px-2 my-1 opacity-80">
                                 {q.barcode.split('').map((char: string, i: number) => {
