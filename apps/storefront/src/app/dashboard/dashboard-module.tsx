...
import {
  TrendingUp, TrendingDown, ShoppingBag, DollarSign, Activity,
  CreditCard, Wallet, Smartphone, Wifi, WifiOff, Clock, CheckCircle
} from 'lucide-react';
import { GetLedgerSummaryQuery, LedgerSummary } from '../../core/queries/ledger.query';
...
export default function DashboardModule() {
  ...
  const [ledgerSummary, setLedgerSummary] = useState<LedgerSummary>({ totalSales: 0, totalPurchase: 0, totalReceivable: 0, totalPayable: 0 });
      ...
      <Row gutter={[20, 20]}>
        <Col xs={24} lg={14}>
          <DashboardCard title="Recent Transactions">
            <div className="overflow-x-auto">
              {recentTx.length === 0 ? (
                <div className="py-10 text-center text-slate-300 font-bold">No transactions yet</div>
              ) : (
                      ...
                      <tr key={i} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${
                            tx.saleType === 'ONLINE' ? 'bg-emerald-100 text-emerald-700' :
        ...
        <Col xs={24} lg={10}>
          <DashboardCard title="Top Selling Products">
            {topSelling.length === 0 ? (
              <div className="py-10 text-center text-slate-300 font-bold">No sales data yet</div>
            ) : (
